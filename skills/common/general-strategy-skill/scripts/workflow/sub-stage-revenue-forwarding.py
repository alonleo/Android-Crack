#!/usr/bin/env python3
"""Reverse-engineering stage: apply explicit reward/local-state edits and build.
...
def execute(ctx):
    result = apply_plan(ctx, 'revenue-forwarding')
    result['build'] = build_project(ctx)
    return result


if __name__ == '__main__':
    raise SystemExit(run_stage('revenue-forwarding', execute))
