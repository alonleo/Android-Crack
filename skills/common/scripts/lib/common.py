...
def normalize_apk_name(raw_name: str) -> str:
    ...
    # driver 传入的 NAME（BMXCycleExtremeBicycleGame），导致子阶段写错目录。
...
def crack_dir():
    """返回 crackings 目录路径。优先用 CRACK_DIR，否则从 NAME+TYPE 推算。"""
    if os.environ.get('CRACK_DIR'):
        return os.environ['CRACK_DIR']
    name = os.environ.get('NAME', '')
    type_arg = os.environ.get('TYPE', '')
    root = repo_root()
    if name and type_arg:
        return str(Path(root) / 'crackings' / type_arg / name)
    elif name:
        return str(Path(root) / 'crackings' / name)
    return ''
def out_dir():
    """返回 raw 解包目录。优先用 OUT，否则从 NAME+TYPE 推算。"""
    if os.environ.get('OUT'):
        return os.environ['OUT']
    name = os.environ.get('NAME', '')
    type_arg = os.environ.get('TYPE', '')
    root = repo_root()
    base = str(Path(root) / 'crackings' / (type_arg + '/' + name if type_arg else name))
    return str(Path(base) / 'raw' / '01-apktool')
