...
/* JADX INFO: loaded from: Il2cppBinary.class */
public class Il2cppBinary {
    ...
    public static void dumpIl2cpp(File in, File out) throws Exception {
        ...
        try {
            OS.exec((String[]) il2cppBinaryCommad.toArray(new String[0]));
    ...
    private static void formatScaffolding(File scaffolding) throws IOException {
        if (scaffolding.exists() && scaffolding.isDirectory()) {
            ...
            for (File f : scaffolding_ARM_spam) {
                if (!f.isDirectory()) {
                    f.delete();
                }
            }
            ...
            for (File f2 : appdataFils) {
                f2.renameTo(new File(scaffolding, f2.getName()));
            }
            delete(appdataDir);
        }
    }
