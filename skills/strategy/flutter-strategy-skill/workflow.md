# Flutter Strategy — Workflow

> 本文件是 flutter-strategy-skill 的**流程操作手册**。
>
...
| 编号 | 子阶段名 | 类型 | 脚本 |
|------|----------|------|------|
| 01 | static-analyze | type-specific | `sub-stage-flutter-static-analyze.py` |
...
**脚本**: `skills/strategy/flutter-strategy-skill/scripts/workflow/sub-stage-flutter-static-analyze.py`

#### 步骤（通用）

| 步骤 | 操作 | 命令/动作 | 验证 |
|------|------|-----------|------|
| 1 | 执行子阶段脚本 | `python3 skills/strategy/flutter-strategy-skill/scripts/workflow/sub-stage-flutter-static-analyze.py <apk> [name]` | 退出码 0 |
...
```bash
# 通用验证（子阶段退出码）
python3 skills/strategy/flutter-strategy-skill/scripts/workflow/sub-stage-flutter-static-analyze.py <apk> [name] && echo "OK"
...
- 所有子阶段共享 `skills/strategy/flutter-strategy-skill/scripts/workflow/sub-stage-flutter-static-analyze.py`
...
实现要点见 [去功能点：flutter](../../common/feature-removal-strategy-skill/references/engine-notes.md#flutter)；步骤、产物与验收见 [统一阶段接口](../../common/feature-removal-strategy-skill/references/stage-contract.md)。
