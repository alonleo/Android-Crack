# Gamemaker Strategy — Workflow

> 本文件是 gamemaker-strategy-skill 的**流程操作手册**。
>
...
实现要点见 [去功能点：gamemaker](../../common/feature-removal-strategy-skill/references/engine-notes.md#gamemaker)；步骤、产物与验收见 [统一阶段接口](../../common/feature-removal-strategy-skill/references/stage-contract.md)。
...
字体资源位于 `assets/game.droid` 的 `FONT` chunk；须核对字体修改实际写回该归档并进入构建包，不能将 Android `res/font/` 视为此 type 的字体交付路径。

**当前产物契约缺口**：10 的 register `outputs` 仅列项目根目录的 `dir-index.yaml` 与 `output-project-manifest.yaml`，11–15 均未声明 `outputs`。应检查这两份已声明清单，同时将实际修改的 `.droid` 资源、构建结果及后续设备证据写入项目阶段记录；清单存在或字段缺失都不能作为汉化验收通过的依据。下文的资源与证据检查要求不代表 register 已声明相应输出文件名。

### 10 — 真机验收（字体）

**脚本**: `skills/common/general-strategy-skill/scripts/workflow/sub-stage-font-device-verify.py`

按当前 register 执行真机验收。该条目未声明 `outputs`；仍须在真实设备覆盖使用 `FONT` 资源的页面，将实际字体加载结果、页面截图和应用运行证据写入项目阶段记录，并按 common 汉化参考检查质量。无法完成真机验收时暂停在 11。

### 11 — 文本汉化操作

**脚本**: `skills/common/general-strategy-skill/scripts/workflow/sub-stage-text-hanization.py`

文本须从 `assets/game.droid` 的 `LANG` chunk 提取、替换并重打包 `.droid`；该路径不依赖 Android `strings.xml`。当前条目未声明 `outputs`；须在项目阶段记录中登记实际修改的归档路径与文本替换结果，并核对构建包中的资源，供 13 真机验收复核。

### 12 — 真机验收（文本）

**脚本**: `skills/common/general-strategy-skill/scripts/workflow/sub-stage-text-device-verify.py`

按当前 register 执行真机验收。该条目未声明 `outputs`；仍须在真实设备进入实际读取 `LANG` 文本的主流程页面，将显示结果、页面截图和应用运行证据写入项目阶段记录，并按 common 汉化参考检查质量。无法完成真机验收时暂停在 13。

### 13 — 图片汉化操作

**脚本**: `skills/common/general-strategy-skill/scripts/workflow/sub-stage-image-hanization.py`

图片文字资源须核对 `assets/game.droid` 中的 `SPRT`（精灵）与 `BGND`（背景）chunk；归档内资源变更应随 `.droid` 重打包。当前条目未声明 `outputs`；须在项目阶段记录中登记实际替换的精灵或背景及归档路径，并核对构建包中的资源，供 15 真机验收复核。

### 14 — 真机验收（图片）

**脚本**: `skills/common/general-strategy-skill/scripts/workflow/sub-stage-image-device-verify.py`

按当前 register 执行真机验收。该条目未声明 `outputs`；仍须在真实设备进入实际加载目标 `SPRT`/`BGND` 资源的页面，将显示结果、页面截图和应用运行证据写入项目阶段记录，并按 common 汉化参考检查质量。无法完成真机验收时暂停在 15。
