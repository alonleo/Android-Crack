# Defold Strategy — Workflow

> 本文件是 defold-strategy-skill 的**流程操作手册**。
>
...
实现要点见 [去功能点：defold](../../common/feature-removal-strategy-skill/references/engine-notes.md#defold)；步骤、产物与验收见 [统一阶段接口](../../common/feature-removal-strategy-skill/references/stage-contract.md)。
...
通用的汉化对象分类、质量边界与风险判断见 [汉化策略参考](../../common/hanization-strategy-skill/SKILL.md)。实际执行、脚本参数、产物检查和验收均以本 workflow 与 defold 的当前 register 为准。

从 XAPK config split 提取 `res/raw-b+zh+Hans/bk_lang_pack.zip`（含字体、标签和图集）并整体替换 base 的 `res/raw/bk_lang_pack.zip`；base 哨兵包不可作为交付语言包。同步将 `bk_locale_marker` 设为 `zh-Hans`，并按本 type `stages/sub-stage-register.yaml` 声明的 `outputs` 检查语言包与阶段产物。
...
文本汉化以完整 `bk_lang_pack.zip` 为主而非 Android `strings.xml`：替换语言包后保留 `bk_locale_marker=zh-Hans`，并按本 type `stages/sub-stage-register.yaml` 声明的 `outputs` 检查语言包及阶段产物；`app_name` 与 manifest label 仍按策略所述同步。

### 12 — 真机验收（文本）

**脚本**: `skills/common/general-strategy-skill/scripts/workflow/sub-stage-text-device-verify.py`

按当前 register 执行真机验收，按本 type `stages/sub-stage-register.yaml` 声明的 `outputs` 检查验证产物并记录状态；另确认 bklang 日志出现 `ENTSCHIEDEN: Sprache zh-hans (Pack aktiv, Sentinel PASS)`。

### 13 — 图片汉化操作

**脚本**: `skills/common/general-strategy-skill/scripts/workflow/sub-stage-image-hanization.py`

图片文字资源随 `bk_lang_pack.zip` 中的标签与图集整体交付；不得将已激活语言包退回 base 哨兵包，并按本 type `stages/sub-stage-register.yaml` 声明的 `outputs` 检查语言包及阶段产物。

### 14 — 真机验收（图片）

**脚本**: `skills/common/general-strategy-skill/scripts/workflow/sub-stage-image-device-verify.py`

按当前 register 执行真机验收，按本 type `stages/sub-stage-register.yaml` 声明的 `outputs` 检查验证产物并记录状态；另确认 bklang 日志报告语言包已激活且 Sentinel PASS。
