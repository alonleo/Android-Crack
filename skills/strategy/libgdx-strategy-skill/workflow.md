# Libgdx Strategy — Workflow

> 本文件是 libgdx-strategy-skill 的**流程操作手册**。
>
...
实现要点见 [去功能点：libgdx](../../common/feature-removal-strategy-skill/references/engine-notes.md#libgdx)；步骤、产物与验收见 [统一阶段接口](../../common/feature-removal-strategy-skill/references/stage-contract.md)。
...
先确定渲染路径：`Typeface.createFromAsset` 路径替换同名 `assets/<font>.ttf`；预渲染 Bitmap Font 则处理 `.fnt` 与配套 `.png` 贴图，不能只替换 ttf。将已确认路径的资源纳入交付，并按本 type `stages/sub-stage-register.yaml` 声明的 `outputs` 检查阶段产物。
...
图片文字资源须先识别 `.atlas` 与配套 `.png` 纹理集，或 `.pack` 容器；按对应资源格式处理并保持资源映射，再按本 type `stages/sub-stage-register.yaml` 声明的 `outputs` 检查阶段产物。
