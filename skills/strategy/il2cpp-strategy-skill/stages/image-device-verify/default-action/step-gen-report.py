...
class GenReportStep(Step):
    def execute(self, steps_results: dict) -> dict:
        ...
        stage_dir = REPO / "crackings" / type_ / name / "stages" / stage_path("image-device-verify")
        stage_dir.mkdir(parents=True, exist_ok=True)

        chk = steps_results.get("image-device-verify.default-action.check-env", {})
        run = steps_results.get("image-device-verify.default-action.run-verify", {})

        report = {
            "stage": "14", "name": "verify-report",
