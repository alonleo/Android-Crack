...
class GenReportStep(Step):
    def execute(self, steps_results: dict) -> dict:
        ...
        stage_dir = REPO / "crackings" / type_ / name / "stages" / stage_path("font-device-verify")
        stage_dir.mkdir(parents=True, exist_ok=True)

        chk = steps_results.get("font-device-verify.default-action.check-env", {})
        run = steps_results.get("font-device-verify.default-action.run-verify", {})

        report = {
            "stage": "10", "name": "verify-report",
