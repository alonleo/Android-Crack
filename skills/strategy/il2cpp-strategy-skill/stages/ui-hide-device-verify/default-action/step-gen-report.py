...
08-ui-hide-device-verify/verify action(通用)。
"""
...
class GenReportStep(Step):
    def execute(self, steps_results: dict) -> dict:
        ...
        stage_dir = REPO / "crackings" / type_ / name / "stages" / stage_path("ui-hide-device-verify")
        stage_dir.mkdir(parents=True, exist_ok=True)

        chk = steps_results.get("ui-hide-device-verify.default-action.check-env", {})
        run = steps_results.get("ui-hide-device-verify.default-action.run-verify", {})

        report = {
            "stage": "08", "name": "verify-report",
